...
def build_named_bar(df_plot, x_col, y_col, bar_color=NAVY, height=360, tickangle=28):
    d = df_plot.copy()
...
def render_main_dashboard(df: pd.DataFrame):
    ...
    with k5:
        kpi_card("🛍️ Vendas no mês", vendas_mes, str(mes), GOLD)
    with k6:
    ...
    with g3:
        ...
        if len(vr) == 0:
            ...
            fig = build_named_bar(vr, COL["raca"], "Total", bar_color=NAVY, height=380, tickangle=28)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with g4:
        ...
        if COL_VENDEDOR:
            ...
            if len(vv) == 0:
                ...
                fig = build_named_bar(vv, COL_VENDEDOR, "Total", bar_color=WINE, height=380, tickangle=28)
                st.plotly_chart(fig, use_container_width=True)
        else:
...
def render_oper_dashboard(df: pd.DataFrame):
    ...
    faturamento_oper = 0.0
    if COL_VALOR and COL_VALOR in f_mes.columns:
        faturamento_oper = float(f_mes[COL_VALOR].apply(brl_to_float).fillna(0).sum())
    ...
    with k4:
        kpi_card("⚠️ Status com erro", erro_mes_oper, "atenção", WINE, value_color="#ef4444" if erro_mes_oper else "#0f172a")
    with k5:
        kpi_card("🛍️ Vendas no mês", vendas_mes_oper, str(mes), GOLD)
    with k6:
        kpi_card("💰 Faturamento", money_br(faturamento_oper), "valor do filhote", NAVY, value_size=28)
